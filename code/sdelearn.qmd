...
df <- import(here('data', 'data.xlsx')) %>%
  ...
  mutate(
    ...
    qyearf = factor(if_else(
      qtype == "Medical",
      cut(qyear, breaks = c(0, 5, 6), labels = c("Other years", "Final year"), right = TRUE),
      cut(qyear, breaks = c(0, 3, 4), labels = c("Other years", "Final year"), right = TRUE)
    ), levels = c("Final year", "Other years")),
    qyear = factor(
...
#     across(c('q52','q53','q56','q57','q58',
#               'q59','q510','q511','q512','q513',
#               'q517','q519','q523'), agree_score, .names = "{col}_score"),
#     across(c('q51','q54','q55','q514','q515',
#               'q516','q518','q520','q521','q522'), agree_score_rev, .names = "{col}_score"),
    across(c('q52','q53',q56:q513,'q517','q519','q523'), agree2g_score, .names = "{col}_score"),
    across(c('q51','q54','q55',q514:q516,'q518',q520:q522), agree2g_score_rev, .names = "{col}_score"),
    ...
    across(c('q72','q75','q76','q77'), tf_score_rev, .names = "{col}_score"),          sum7_score = rowSums(across(q71_score:q77_score), na.rm = TRUE),
    ...
    sum8_score = rowSums(across(q81_score:q811_score), na.rm = TRUE),
  ...
  set_variable_labels(
    ...
    qtype = 'Type',
    qyear = 'Year of study',
    qyearf = 'Year of study',
    q11 = 'Age (years)',
    agegrp = 'Age group',
    q12 = 'Gender',
    q13 = 'Place of origin (Region)',
    q21 = '2.1 Have you ever contacted/taken care HIV patients?',
...
df14 <- df %>%
  ...
  drop_na() %>% 
  set_variable_labels(
    q21 = 'Have you ever contacted/taken care HIV patients?',
...
tbl_summary(
  df,
  by = qtype,
  type = list(q11 ~ 'continuous2', q21 ~ "categorical"),
  statistic = q11 ~ c("{mean} ({sd})", "{median} ({p25}, {p75})", "{min}, {max}"),
  digits = list(q11 ~ c(1, 1, 0, 0, 0, 0, 0),
                all_categorical() ~ c(0, 1)),
  include = c(q12, q11, agegrp, q13, quniv, qyear, q25, q21)
) %>%
  add_overall() %>%
  bold_labels() %>%
...
#### Table 3A: Worried about getting HIV infection (n = `r n`)
...
#### Table 3B: Worried about getting HIV infection (n = `r n3b`)
...
#### Table 4: Professional behaviors - Q3a: How worried (n = `r n`)
...
#### Table 5: Professional behaviors - Q3b: Intent to discriminate (n = `r n`)
...
#### Table 6: Professional community norms (n = `r n`)
...
#### Table 7: Healthcare policies (n = `r n`)
...
#### Table 8: Caring for different populations (n = `r n`)
...
#### Table 10: HIV Transmission (n = `r n`)
...
#### Table 11a: Standard precaution knowledge (n = `r n`)
...
#### Table 11b: Standard precaution knowledge (n = `r n`)
...
#### Table 12: Protective measures (n = `r n`)
...
#### Table 14: Factor **associated with intention to discriminate while performing invasive procedures** (n = `r n`)

```{r t14}
t_stat <- tbl_summary(
  df14,
  by = q3b,
  type = c('q11', ends_with('_score')) ~ 'continuous',
...
# fullmodel <- glm(
#   q3b ~ q12 + quniv + qyearf + sum7_score + sum3a_score + sum2_score,
#   family = "binomial",
#   data = df14
# )
# summary(fullmodel)
...
# 
# model1 <- glm(
#   q3b ~ quniv + qyearf + sum7_score + sum3a_score + sum2_score,
#   family = "binomial",
#   data = df14
# )
# summary(model1)
...
# 
model2 <- glm(
  q3b ~ quniv + qyearf + sum7_score + sum3a_score,
  family = "binomial",
  data = df14
)
# summary(model2)
...
t_multi <- tbl_regression(model2, exponentiate = TRUE) %>%
  bold_p(0.05)
